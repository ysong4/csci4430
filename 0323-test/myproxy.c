...
void* workerThread(void* args){
    ...
    int id = args2.id;
    char bufferA[REQUEST_SIZE]; //= (char *)malloc(sizeof(char) * REQUEST_SIZE);
    char bufferB[REQUEST_SIZE];
    char bufferC[REQUEST_SIZE];
    ...
    while(1){
        ...
        int result;
        memset(bufferA, 0, REQUEST_SIZE);
        result = recv(browser_sd, bufferA, REQUEST_SIZE, 0);
        if (result == 0){
            continue;
        }
        ...
        // while((result = recv(browser_sd, bufferB, REQUEST_SIZE, 0))>0) {
            // printf("request id[%d]: \n%s\n", k, bufferB);
            // k++;
        // }
        memset(bufferB, 0, REQUEST_SIZE);
        ...
        char ** lines = splitString(bufferA, 0);
        ...
        if(existCache == 1 && supportedFileType == 1){
            ...
            if(haveIMS == 0 && haveCache == 0){
                ...
                caseType = 3;
                char ** headerlines = splitString(bufferC, 0);
                ...
                memset(requestBuffer, 0, REQUEST_SIZE);
                strcpy(requestBuffer, headerlines[0]);
                strcat(requestBuffer, "\r\n");

                while(headerlines[b] != NULL){
                    strcat(requestBuffer, headerlines[b]);
                    strcat(requestBuffer, "\r\n");
                    b++;
                }
                strcat(requestBuffer, tmpline);
                strcat(requestBuffer, "\r\n");
                strcat(requestBuffer, "\r\n");
        ...
        if (existCache == 0){
            ...
            //printf("\n%s\n", bufferA);
            int res = send(server_sd, bufferA, strlen(bufferA), MSG_NOSIGNAL);
            //printf("\n%d\n", strlen(bufferA));
            if(res<0){
                ...
                while (1) { // receive byte by byte
                    res = recv(server_sd, &bufferA[n], 1, 0);
                    if (res < 0){
                        perror("Error in receiving HTTP response!");
                        break;
                    }
                    if (n > 4 && strstr(bufferA, "\r\n\r\n") != NULL) {
                        break;
                    }
                    n++;
                }
                ...
                if (strstr(statCode, "200") != NULL && supportedFileType) {
                    ...
                    fclose(fp);
                    pthread_mutex_unlock(&mutex);
                } else if ((strstr(statCode, "304") != NULL) && supportedFileType && (caseType == 3 || caseFourOverwriteIMS == 1)) {
